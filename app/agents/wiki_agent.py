...
class ReActAgent:
    ...
    def _setup_loggers(self) -> None:
        ...
        if not self.mcp_logger.handlers:
            fh_mcp = logging.FileHandler(
                os.path.join(log_dir, "mcp.log"), encoding="utf-8"
            )
        ...
        if not self.graph_logger.handlers:
            fh_graph = logging.FileHandler(
                os.path.join(log_dir, "graph_state.log"), encoding="utf-8"
            )
    ...
    async def run(
        ...
        try:
            ...
            return await self._run_mock(
                query, str(e), return_tools, run_id, return_mcp_logs
            )
    ...
    def _get_calc_params(self) -> StdioServerParameters:
        ...
        return StdioServerParameters(
            command=sys.executable, args=[calc_server_path, "--stdio"]
        )
    ...
    def _build_system_prompt(self, tools_map: dict) -> str:
        ...
        for name, (session, tool) in tools_map.items():
            tools_list_desc.append(
                f"- {name}: {tool.description}. Схема аргументов: {json.dumps(tool.inputSchema)}"
            )
    ...
    def _parse_action(self, text: str) -> Optional[tuple[str, str]]:
        ...
        if not action_match:
            action_match = re.search(
                r"Action:\s*(\w+)\((.*?)\)", text, re.DOTALL | re.IGNORECASE
            )
        if action_match:
    ...
    def _parse_final_answer(self, text: str) -> Optional[str]:
        ...
        action_match = re.search(
            r"Action:\s*final_answer\s*:\s*(.*)", text, re.DOTALL | re.IGNORECASE
        )
        if not action_match:
            action_match = re.search(
                r"Action:\s*final_answer\((.*)\)", text, re.DOTALL | re.IGNORECASE
            )
                ...
                async with stdio_client(calc_params) as (calc_read, calc_write):
                    async with ClientSession(calc_read, calc_write) as calc_session:
                        ...
                        async def agent_node(state: AgentState) -> dict:
                            ...
                            response = await self.inference_module.run(
                                messages=messages
                            )
                            return {
                        ...
                        async def tools_node(state: AgentState) -> dict:
                            ...
                            if not parsed_action:
                                return {
                                    ...
                                    + [
                                        {
                                            "role": "user",
                                            "content": "Ошибка: не найден формат Action: название_инструмента: аргументы.",
                                        }
                                    ]
                                }
                            ...
                            if tool_name not in tools_map:
                                return {
                                    ...
                                    + [
                                        {
                                            "role": "user",
                                            "content": f"Ошибка: неизвестный инструмент {tool_name}.",
                                        }
                                    ]
                                }
                                ...
                                if tool.name in [
                                    ...
                                    return {
                                        ...
                                        + [
                                            {
                                                "role": "user",
                                                "content": f"Ошибка: не удалось распарсить аргументы инструмента {tool.name} как JSON.",
                                            }
                                        ]
                                    }
                            ...
                            try:
                                self.mcp_logger.info(
                                    f"[{run_id}] Call: {tool.name} | Args: {arguments}"
                                )
                                ...
                                if (
                                    ...
                                    observation = await self.inference_module.run(
                                        messages=extract_messages
                                    )

                                self.mcp_logger.info(
                                    f"[{run_id}] Res: {observation.replace(chr(10), ' ')}"
                                )
                            except Exception as e:
                                observation = f"Ошибка при вызове инструмента: {str(e)}"
                                self.mcp_logger.error(
                                    f"[{run_id}] Err: {str(e).replace(chr(10), ' ')}"
                                )
                        ...
                        async def self_check_node(state: AgentState) -> dict:
                            ...
                            return {
                                ...
                                + [
                                    {
                                        "role": "user",
                                        "content": "Самопроверка не пройдена. Пожалуйста, проверьте рассуждения.",
                                    }
                                ],
                                "final_answer": "",
                            }
                        ...
                        workflow.add_conditional_edges(
                            "Validate_Answer",
                            self_check_router,
                            {"end": END, "LLM_Reasoning": "LLM_Reasoning"},
                        )
                            ...
                            async for event in graph.astream(initial_state):
                                for node_name, node_state in event.items():
                                    ...
                                    if (
                                        ...
                                        self.graph_logger.info(
                                            f"[{run_id}] Msg: [{last_msg.get('role', 'unknown')}] {msg_short}"
                                        )
                                    if (
                                        "final_answer" in node_state
                                        ...
                                        self.graph_logger.info(
                                            f"[{run_id}] Final: {ans_short}"
                                        )

                                    current_state.update(node_state)
                        except Exception as e:
                            self.graph_logger.error(
                                f"[{run_id}] Ошибка во время выполнения графа: {str(e)}",
                                exc_info=True,
                            )
                            raise e
