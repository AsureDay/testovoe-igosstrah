...
async def evaluate_answer(
    validator_module: InferenceModule, query: str, reference: str, agent_answer: str
) -> ValidationResult:
    """
    ...
    try:
        ...
        response = await validator_module.run(
            query=prompt, response_format=response_format
        )
        ...
        if json_match:
            ...
            if start != -1 and end != -1:
                ...
                return ValidationResult(
                    score=1, reason="Failed to parse JSON from response"
                )
...
async def run_validation():
    ...
    filepath = os.path.abspath(
        os.path.join(os.path.dirname(__file__), "validation_QA.jsonl")
    )
    ...
    async def process_test_case(case, agent, inference, prompt_name):
        ...
        async with semaphore:
            ...
            agent_answer, used_tools = await agent.run(
                query, return_tools=True, run_id=run_id
            )
            print(
                f"[{run_id}][{prompt_name}] Agent Answer for '{query}': {agent_answer}"
            )
            print(f"[{run_id}][{prompt_name}] Used Tools for '{query}': {used_tools}")

            val_result = await evaluate_answer(
                inference, query, reference, agent_answer
            )
            print(
            ...
            for t in expected_tools:
                if t not in used_tools and not (
                    t == "search_wikipedia" and "wikipedia_search" in used_tools
                ):
            ...
            print(
                f"[{run_id}][{prompt_name}] Tool Score: {tool_score}, Tool Reason: {tool_reason}\n"
            )

            ans_short = agent_answer.replace("\n", " ")
            val_logger.info(
                f"[{run_id}][{prompt_name}] Ans: {ans_short} | Score: {val_result.score} | ToolScore: {tool_score}"
            )
    ...
    for model_name in models:
        for prompt_name, prompt_template in prompts.items():
            ...
            agent = ReActAgent(
                inference_module=inference, system_prompt_template=prompt_template
            )

            tasks = [
                process_test_case(case, agent, inference, prompt_name)
    ...
    output_path = os.path.abspath(
        os.path.join(os.path.dirname(__file__), "validation_results.json")
    )
